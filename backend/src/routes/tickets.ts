...
import { db } from '../db'
import { tickets, ticketAnalysis, assignments, managers, businessUnits } from '../db/schema'
...
export const ticketsRoutes = new Elysia({ prefix: '/tickets' })
  ...
  .get('/', async ({ query }) => {
    const { limit = '50', offset = '0', office, segment, ticket_type, sentiment, language } = query
    ...
    const rows = await db
      ...
      .where(
        and(
          ...
          ticket_type ? eq(ticketAnalysis.ticketType, ticket_type) : undefined,
