...
export function SegmentBadge({ segment }: { segment: string | null }) {
  ...
  const cls =
    segment === 'VIP' ? 'badge-vip' :
    segment === 'Priority' ? 'badge-priority' :
    'badge-mass'
...
export function SentimentBadge({ sentiment }: { sentiment: string | null }) {
  ...
  const cls =
    sentiment === 'Позитивный' ? 'badge-positive' :
    sentiment === 'Негативный' ? 'badge-negative' :
    'badge-neutral'
...
const LANG_FLAGS: Record<string, string> = { RU: '🇷🇺', KZ: '🇰🇿', ENG: '🇬🇧', EN: '🇬🇧' }
export function LangBadge({ lang }: { lang: string | null }) {
  if (!lang) return null
  return <span className="badge badge-lang">{LANG_FLAGS[lang] ?? ''} {lang}</span>
}
