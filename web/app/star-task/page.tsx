...
import { Sparkles, Send, ChevronDown, ChevronUp, Database, Zap, TrendingUp, User, Bot } from 'lucide-react'
...
export default function StarTaskPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      role: 'assistant',
      content: 'Привет! Я аналитик FIRE. Спроси меня статистику по обращениям, и я нарисую график, или просто задай вопрос по данным.',
    }
  ])
  ...
  async function handleSubmit(forcedQuery?: string) {
    ...
    try {
      ...
      const historyToSend = [...messages, newMsg].map(m => ({ role: m.role, content: m.content }))
      ...
      setMessages(prev => [
        ...prev, 
        { id: (Date.now() + 1).toString(), role: 'assistant', content: data.text || 'Нет ответа', result: data }
      ])
    } catch (e: unknown) {
      setMessages(prev => [
        ...prev, 
        { id: (Date.now() + 1).toString(), role: 'assistant', content: `Ошибка: ${e instanceof Error ? e.message : 'Неизвестная ошибка'}` }
      ])
    }
    setLoading(false)
  }
    ...
    <div className={`page ${s.pageChat}`}>
      ...
      <div className={s.chatHeader}>
        ...
        <div>
          ...
          <div className={s.caps} style={{ marginTop: 4 }}>
            <span className={s.cap}><Database size={12} />PostgreSQL Agent</span>
            <span className={s.cap}><Zap size={12} />Qwen2.5 JSON Mode</span>
          </div>
        </div>
      </div>
      ...
      <div className={s.chatScroll}>
        <div className={s.chatMessages}>
          {messages.map(m => (
            <div key={m.id} className={`${s.messageRow} ${m.role === 'user' ? s.messageRowUser : s.messageRowAssistant}`}>
              <div className={s.avatar}>
                {m.role === 'user' ? <User size={16} /> : <Sparkles size={16} />}
              </div>
              <div className={m.role === 'user' ? s.bubbleUser : s.bubbleAssistant}>
          ...
          {loading && (
            <div className={`${s.messageRow} ${s.messageRowAssistant}`}>
              <div className={s.avatar}><Sparkles size={16} /></div>
      ...
      <div className={s.chatFooter}>
        <div className={s.chipsScroll}>
          {EXAMPLES.map(ex => (
            <button key={ex.text} className={s.chip} onClick={() => handleSubmit(ex.text)}>
              {ex.icon} {ex.text}
            </button>
          ))}
        </div>
        ...
        <div className={s.searchRow}>
          <div className={s.searchInputWrap}>
            <input
              ...
              placeholder="Спроси о тикетах, менеджерах или филиалах..."
              value={query}
          ...
          <button className={s.searchBtn} onClick={() => handleSubmit()} disabled={!query.trim() || loading}>
            <Send size={15} />
          </button>
        </div>
      </div>
...
function ChartBlock({ data }: { data: NonNullable<StarTaskResult['data']> }) {
    ...
    <div className={s.chartBlock}>
      <div className="card" style={{ marginTop: 12 }}>
        <div className="card-header">
          <h3 className="card-title" style={{ fontSize: 14 }}>{data.chartTitle}</h3>
          <span className="badge badge-new" style={{ fontSize: 10, textTransform: 'uppercase' }}>
            {data.chartType}
          </span>
        </div>
      ...
      <button className={s.sqlToggleChat} onClick={() => setSqlOpen(v => !v)}>
        <Database size={12} />
        <span>{sqlOpen ? 'Скрыть SQL' : 'Показать сгенерированный SQL'}</span>
...
function DynamicChart({ dataObj }: { dataObj: NonNullable<StarTaskResult['data']> }) {
  const data = dataObj.rows.map(row => {
    const obj: Record<string, unknown> = {}
    dataObj.columns.forEach((col, i) => { obj[col] = (row as unknown[])[i] })
    return obj
  })
    ...
    return (
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data}>
          ...
          <Tooltip />
          <Line type="monotone" dataKey={valueKey} stroke="#2563EB" strokeWidth={2} dot={{ r: 4 }} />
        </LineChart>
      </ResponsiveContainer>
    )
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={isVertical ? Math.max(260, data.length * 32) : 260}>
      <BarChart data={data} layout={isVertical ? 'vertical' : 'horizontal'} margin={{ left: 16, right: 24, top: 8, bottom: 8 }}>
        {isVertical ? (
          <>
        ...
        <Tooltip />
        <Bar dataKey={valueKey} fill="#2563EB" radius={isVertical ? [0, 4, 4, 0] : [4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  )
}
